...
async function createDocumentcategory(req, res) {
    try {
        const { document_name, document_type, side, document_number, Status } = req.body;
        ...
        res.status(201).json({ message: "Document category created successfully", documentCategory });
    } catch (error) {
        console.error(error);
...
async function searchDocumentcategory(req, res) {
    try {
        ...
        if (document_name !== undefined) {
            searchCriteria.document_name = { $regex: new RegExp(`^${document_name}$`, 'i'), };
        }
        if (document_type !== undefined) {
            searchCriteria.document_type = { $regex: new RegExp(`^${document_type}$`, 'i'), };
        }
        if (side !== undefined) {
...
async function deleteDocumentcategory(req, res) {
    try {
        ...
        const existingDocumentCategory = await DocumentCategory.findOne({ document_name });
...
async function updateDocumentcategory(req, res) {
    try {
        const { id, document_name, document_type, side, document_number, Status } = req.body;
